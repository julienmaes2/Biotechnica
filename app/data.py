...
_YF_RES_MAP = {"D":("1d",None), "60":("60m","60d"), "30":("30m","60d"), "15":("15m","60d"), "5":("5m","60d"), "1":("1m","7d")}

def list_us_symbols_finnhub(api_key: str, industry_filters=("Biotechnology","Pharmaceuticals")) -> pd.DataFrame:
    url = f"{BASE}/stock/symbol"; params = {"exchange":"US", **_auth(api_key)}
...
def list_symbols_from_inputs(seed_csv_path: str, uploaded_df: pd.DataFrame = None, pasted: str = None, sheet_url: str = "") -> pd.DataFrame:
    # Google Sheet CSV export
    if sheet_url and "docs.google.com" in sheet_url:
        try:
            csv_url = google_sheet_to_csv_url(sheet_url); gdf = pd.read_csv(csv_url)
            col = [c for c in gdf.columns if c.lower()=="symbol"]
            if col:
                syms = gdf[col[0]].astype(str).str.upper().str.strip().unique().tolist()
    ...
    if pasted:
        ...
        if syms: return pd.DataFrame({"symbol": list(dict.fromkeys(syms))})
    # Seed
    seed = pd.read_csv(seed_csv_path); seed["symbol"] = seed["symbol"].astype(str).str.upper().str.strip()
...
def google_sheet_to_csv_url(url: str) -> str:
    ...
    try: doc_id = url.split("/d/")[1].split("/")[0]
...
def get_candles_finnhub(api_key: str, symbol: str, resolution: str = "D", lookback_days: int = 120) -> pd.DataFrame:
    to_ts = int(time.time()); from_ts = to_ts - lookback_days*86400
    ...
    df = pd.DataFrame({"t": pd.to_datetime(data["t"], unit="s", utc=True), "o":data["o"], "h":data["h"], "l":data["l"], "c":data["c"], "v":data["v"]})
...
def get_history_yf(symbol: str, start: str, end: str) -> pd.DataFrame:
