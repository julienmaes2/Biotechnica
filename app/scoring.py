...
def moonshot_score_row(row, weights):
    score = 0.0
    # 1) Price breakout: close > 1.05 * 10-day SMA
    price_breakout = float(row.get("price_breakout", 0))
    # 2) RSI momentum: scaled 0..1 when between 55..80; lower if oversold <30 (optional)
    ...
    # 3) MACD signal > 0 and hist rising
    macd_hist = row.get("macd_hist", 0.0)
    macd_up = 1.0 if macd_hist > 0 else 0.0

    # 4) Volume spike vs 30-day avg
    vol_spike = row.get("vol_spike", 0.0)
    vol_component = min(max(vol_spike - 1.0, 0.0), 2.0) / 2.0  # cap at 2x

    # 5) Catalyst proximity boolean/decay (0..1)
